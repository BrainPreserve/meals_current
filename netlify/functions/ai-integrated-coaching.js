...
exports.handler = async (event) => {
  try {
    ...
    const messages = [
      { role: 'system', content: [
        'You are Clinical AI Coaching for a brain-health program.',
        'Integrate biomarkers with meal generator outputs.',
        'Audience: older adults; tone: professional, empowering; non-medical.',
        'Prioritize insulin sensitivity, GV/TIR, BP variability, HRV, sleep, inflammation.'
      ].join(' ')},
      { role: 'user', content: [
        `CLIENT_ID: ${client_id}`,
        `BIOMARKERS:\n${biomarkerText || '(none mirrored yet)'}`,
        `MGA_RECIPES_COUNT: ${mga.recipe_count ?? ''}`,
        `MGA_RECIPES_HTML:\n${mga.meals_html || ''}`,
        `MGA_NUTRITION_TABLES_HTML:\n${mga.tables_html || ''}`,
        'TASK: Provide integrated AI COACHING with 5–8 precise, food-based actions tied to the biomarker patterns.'
      ].join('\n\n')}
    ];
